...
class MedicalRecordServiceTest {
    ...
    void deleteMedicalRecordTest() throws IOException, ParseException {
        ...
        MedicalRecordModel medicalRecordModelTest = new MedicalRecordModel("Bi", "Bam", "03/05/1974", medications, allergies);
        medicalRecordService.createMedicalRecord(medicalRecordModelTest);

        MedicalRecordModel medicalRecordModel = medicalRecordService.getMedicalRecord("Bi", "Bam");
        medicalRecordService.deleteMedicalRecord(medicalRecordModel);

        Assert.assertFalse(medicalRecordService.getMedicalRecordsList().contains(medicalRecordModel));
    }
    ...
    void createMedicalRecordTest() throws IOException, ParseException {
        List<String> medications = new ArrayList<>();
        List<String> allergies = new ArrayList<>();
        MedicalRecordModel medicalRecordModel = new MedicalRecordModel("Bi", "Bam", "03/05/1974", medications, allergies);
        medicalRecordService.createMedicalRecord(medicalRecordModel);

        Assert.assertTrue(medicalRecordService.getMedicalRecordsList().contains(medicalRecordModel));
    ...
    void updateMedicalRecordTest() throws IOException, ParseException {
        ...
        MedicalRecordModel medicalRecordModelTest = new MedicalRecordModel("Bi", "Bam", "13/13/1666", medications, new ArrayList<>());
        medicalRecordService.createMedicalRecord(medicalRecordModelTest);

        Assert.assertTrue(medicalRecordService.getMedicalRecord("Bi", "Bam").getAllergies().isEmpty());

        MedicalRecordModel medicalRecordModel = medicalRecordService.getMedicalRecord("Bi", "Bam");
        medicalRecordModel.setAllergies(allergies);
        medicalRecordService.updateMedicalRecord(medicalRecordModel);

        Assert.assertEquals("[nillacilan]", medicalRecordModel.getAllergies().toString());
